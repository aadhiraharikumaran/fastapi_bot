...
CORSMiddleware,
allow_origins=["*"],
@@ -1217,23 +31,29 @@ async def lifespan(app: FastAPI):
allow_headers=["*"],
)
...
@@ -1248,198 +68,232 @@ class MessageRequest(BaseModel):
...
try:
        if supabase:
            ...
            supabase.table(table).insert(serialized_data).execute()
            logger.debug(f"{request_id}:-Successfully logged to Supabase table '{table}'")
        if not supabase:
